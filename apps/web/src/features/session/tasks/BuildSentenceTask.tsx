  ...
  return (
    <div>
      ...
      <Card hover={false} className="mt-3 text-center">
        <p className="text-hero-sm font-extrabold text-ink">{payload.promptTranslation}</p>
      </Card>

      <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-muted">Tap the tiles in order</p>
      <div className="mt-2 flex min-h-[52px] flex-wrap items-center gap-2 rounded-xl border-2 border-dashed border-line p-3">
        {selectedTiles.map((tile, i) => (
          <button
            ...
            className="rounded-lg bg-teal px-3 py-1.5 text-sm font-semibold text-white disabled:opacity-60"
          >
            {tile.surface}
          </button>
        ))}
        {payload.trailingPeriodPinned && <span className="font-semibold text-ink">.</span>}
      </div>
        ...
        {available.map((tile) => (
          ...
          <div key={tile.id} className="relative">
            <button
              ...
              className="rounded-lg border-2 border-line bg-surface py-1.5 pl-3 pr-6 text-sm font-semibold text-ink hover:border-teal hover:bg-teal-soft disabled:opacity-60"
            >
              {tile.surface}
            </button>
            <SpeakButton
