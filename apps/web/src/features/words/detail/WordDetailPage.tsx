...
export function WordDetailPage() {
  ...
  if (isError || !word) {
    return <p className="py-12 text-center text-sm text-muted">Couldn't find that word.</p>;
  }
  ...
  function handleDelete() {
    ...
    deleteWord.mutate(id, { onSuccess: () => navigate('/words') });
  }

  return (
    <div className="lg:ml-auto lg:max-w-md">
      <Link to="/words" className="inline-flex items-center gap-1 text-sm font-semibold text-muted hover:text-deep">
        ...
        {!isEditing ? (
          <>
            ...
            <div className="mt-4 flex gap-2">
              <Button variant="outline" className="!px-4 !py-1.5 text-sm" onClick={startEditing}>
                Edit
              </Button>
              <Button variant="coral" className="!px-4 !py-1.5 text-sm" onClick={handleDelete}>
                Delete
              </Button>
            </div>
          </>
          ...
          <div className="mt-4 flex flex-col gap-3">
            <div>
              <label className="text-xs font-semibold text-muted">Your translation</label>
              <Input value={customTranslation} onChange={(e) => setCustomTranslation(e.target.value)} className="mt-1" />
            </div>
            <div>
            ...
            <div className="flex gap-2">
              <Button onClick={saveEdits}>{updateWord.isPending ? 'Saving…' : 'Save'}</Button>
      ...
      <Card className="mt-4" hover={false}>
        ...
        <div className="mt-4">
          <MasteryLadder level={word.level} />
        </div>
        <Button variant="light" className="mt-5 w-full justify-center" disabled>
          ...
          <div className="mt-3 flex flex-col gap-2">
            {word.attempts.map((attempt) => (
              <div key={attempt.id} className="flex items-center justify-between text-sm">
                <span className="text-deep">{attempt.taskType}</span>
                <span className="text-muted">{attempt.result}</span>
              </div>
            ))}
          </div>
        )}
